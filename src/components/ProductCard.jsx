  ...
  return (
    <article
      className="rounded-xl border border-brand-border bg-brand-surface shadow-2xl 
                 overflow-hidden transition duration-500 transform hover:scale-105 hover:shadow-2xl"
    >
      ...
      <div className="p-6 transition-colors duration-500">
        <h3 className="font-sans text-brand-text text-lg">{name}</h3>
        <p className="mt-1 text-sm text-brand-muted">Eau de parfum</p>
        <div className="mt-4 flex items-center justify-between">
          <span className="font-medium text-brand-text">{price}</span>
          <button
            onClick={handleAdd}
            className="px-5 py-2 rounded-full border border-brand-border text-brand-text 
                       font-bold text-base hover:bg-brand-accent hover:text-black transition-colors duration-500"
          >
            Adicionar
          </button>
        </div>
