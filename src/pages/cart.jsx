  ...
  return (
    <main className="min-h-screen bg-brand-bg text-brand-text transition-colors duration-500">
      <div className="mx-auto max-w-6xl px-4 py-12">
        <h2 className="font-display text-3xl mb-6">Meu Carrinho</h2>

        {items.length === 0 ? (
          <p className="text-brand-muted">Seu carrinho está vazio.</p>
        ) : (
          <div className="space-y-6">
            {items.map((item) => (
              <div
                key={item.id}
                className="flex items-center gap-4 bg-brand-surface rounded-xl shadow-soft p-4 border border-brand-border transition-colors duration-500"
              >
                <img
                  src={item.image}
                  alt={item.name}
                ...
                <div className="flex-1">
                  ...
                  <p className="text-brand-muted">{item.price}</p>
                </div>
